...
async function main() {
    ...
    const admin = await prisma.user.create({
        data: { name: "IT Admin", email: "admin@steakz.test", passwordHash, role: client_1.Role.ADMIN }
    });
    await prisma.user.create({
        data: { name: "System Admin", email: "system.admin@steakz.test", passwordHash, role: client_1.Role.ADMIN }
    });
    await prisma.user.create({
        data: { name: "UK Country Manager", email: "country@steakz.test", passwordHash, role: client_1.Role.HEADQUARTER_MANAGER }
    });
    const branchStaff = [];
    for (const branch of ukBranches) {
